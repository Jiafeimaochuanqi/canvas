...
Item {
    ...
    function drawPoint(ctx,pos,pointColor="#FF0000"){
    ...
    function drawControlBar(ctx,control,left,right,lineColor="#FF0000"){
        drawPoint(ctx,left,lineColor)
        //drawPoint(ctx,control,lineColor)
        drawPoint(ctx,right,lineColor)
        ctx.strokeStyle = lineColor
        ctx.lineWidth = 2
        if(control.length>0){
            ctx.beginPath()
    ...
    Canvas {
        ...
        onPaint: {
            ...
            drawPoint(ctx,cppObject.input,lineColor.model[cppObject.parameterizationType])
